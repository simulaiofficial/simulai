...
def deserialize_page(json_page):
    ...
    page_blocks = PageBlocks(name="", avatarUrl=None, botName=None, context=None, blocks=blocks_json)
    # Use get_blocks to deserialize the blocks
    name, bot_name, avatar_url, context, deserialized_blocks = get_blocks(page_blocks)
    ...
    for workspace_bot_json in workspace_bots_json:
        bot_page_blocks = PageBlocks(name="", avatarUrl=None, botName=None, context=None, blocks=workspace_bot_json['blocks'])
        print('Processing:', workspace_bot_json['name'])
        n, bot_name, avatar_url, context, deserialized_bot_blocks = get_blocks(bot_page_blocks)
...
async def save_data(dst: str, pay = Depends(get_blocks)):
    name, bot_name, avatar_url, context, blocks = pay
